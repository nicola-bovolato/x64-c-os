...
void unmap_page(
    page_table_t*      table4_ptr,
    page_entry_t       page,
    deallocate_frame_t deallocate_frame,
    bool               panic_on_empty
);
...
void init_paging(uint32_t* multiboot_header) {
    ...
    // Map temporarily new table4 to be able to access it
    {
        ...
        new_phys_table4_ptr->entries[PAGE_ENTRIES - 1].fields.address
            = (size_t)new_phys_table4_ptr / FRAME_SIZE;
    ...
    {
        volatile const page_table_t* phys_table4_ptr = read_cr3();
...
void* get_physical_address(page_table_t* table4_ptr, void* virtual) {
    ...
    return (void*)((size_t)table1_entry.fields.address * FRAME_SIZE + offset);
}
...
void identity_map(
    ...
    page_entry_t page   = {.bits = 0};
    page.fields.address = (size_t)frame_ptr / FRAME_SIZE;
...
void map_page_to_frame(
    ...
    if (table1_entry->bits != 0)
        PANIC(
            ...
            page.bits,
            (size_t)table1_entry->fields.address * FRAME_SIZE
        );
...
void unmap_page(
    ...
    void* frame_ptr = (void*)((size_t)table1_entry->fields.address * FRAME_SIZE);
