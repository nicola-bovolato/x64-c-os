...
mem_region_t get_mem_region() {
    uint8_t* mem_end = get_ptr_from_tag(MULTIBOOT_TAG_TYPE_BASIC_MEMINFO, get_meminfo_end_ptr);
    return (mem_region_t) {0, mem_end};
}
mem_region_t get_multiboot_mem_region() {
    return (mem_region_t) {(uint8_t*)multiboot_info_ptr, (uint8_t*)multiboot_info_ptr + *(multiboot_info_ptr)};
}

size_t get_elf_sections_number() {
    return (size_t) get_ptr_from_tag(MULTIBOOT_TAG_TYPE_ELF_SECTIONS, get_elf_sections_number_as_ptr);
}

multiboot_elf_section_t* get_elf_sections() {
    return (multiboot_elf_section_t*) get_ptr_from_tag(MULTIBOOT_TAG_TYPE_ELF_SECTIONS, get_elf_sections_ptr);
}
