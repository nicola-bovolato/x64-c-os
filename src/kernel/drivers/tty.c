...
void clear_screen() {
    ...
    for (uint16_t* pointer = (uint16_t*)VGA_MEM_START; pointer <= vga_mem_end; pointer++) {
...
static inline void set_row(size_t row) {
    ...
    cursor = (uint16_t*)(VGA_MEM_START + get_col() * 2);
...
static inline void set_col(size_t col) {
    ...
    cursor = (uint16_t*)(VGA_MEM_START + (get_row() * VGA_COLS * 2));
...
static inline void scroll(int rows) {
    ...
    for (uint16_t* pointer = (uint16_t*)(VGA_MEM_START + to_copy); pointer <= vga_mem_end;
         pointer++) {
        *pointer = (VGA_BLACK << 4 | VGA_WHITE) << 8 | 0x20;
    }
}
