...
static uint16_t *const vga_mem_end = VGA_MEM + (VGA_ROWS * VGA_COLS - 1) * 2;

static uint16_t *cursor = VGA_MEM;
...
void clear_screen(){
    ...
    for( uint16_t *pointer = VGA_MEM; pointer < vga_mem_end; pointer++ ) {
...
static inline void print_char_internal(char to_print) {
    *cursor = vga_color << 8 | to_print;
    ...
    cursor = (cursor < vga_mem_end) ? cursor + 1 : VGA_MEM;
}

static inline void set_row(size_t row) {

    // reset the cursor to the start of vga memory + cols
    cursor = VGA_MEM + get_col() * 2;
...
static inline void set_col(size_t col) {
    ...
    cursor = VGA_MEM + (get_row() * VGA_COLS * 2);
